...
class GPTChatTool:
    ...
    def ii_push_record(self, in_record: object) -> bool:
        ...
        gpt_result = self.get_gpt_response(
            prompt, model, api_key, max_tokens,
    ...
    def count_tokens(text, model):
        tokens = openai.Completion.tokens(text, model)
        return len(tokens)

    # Add split_text function
    def split_text(text, model, max_tokens):
        ...
        if tokens <= max_tokens:
            ...
            current_text = ''
            for word in words:
                temp_text = current_text + ' ' + word
                if count_tokens(temp_text, model) <= max_tokens:
    ...
    def save_to_csv(df, output_file):
        ...
        self.input_column_name = configuration.getElementsByTagName("InputColumn")[
            0].firstChild.data.strip()
        self.output_column_name = configuration.getElementsByTagName("OutputColumn")[
            0].firstChild.data.strip()
        self.prompt_column_name = configuration.getElementsByTagName("PromptColumn")[
            0].firstChild.data.strip()
        self.prompt_length = int(configuration.getElementsByTagName(
            "PromptLength")[0].firstChild.data.strip())
        self.engine = configuration.getElementsByTagName(
            "Engine")[0].firstChild.data.strip()
        self.model = configuration.getElementsByTagName(
            "Model")[0].firstChild.data.strip()
        self.api_key = configuration.getElementsByTagName(
            "APIKey")[0].firstChild.data.strip()
        self.google_key = configuration.getElementsByTagName(
            "GoogleKey")[0].firstChild.data.strip()
        self.google_cx = configuration.getElementsByTagName(
            "GoogleCX")[0].firstChild.data.strip()
    ...
    def ii_push_record(self, in_record: object, prompt_column: str, input_column: Optional[str], output_column: str) -> bool:
        ...
        if input_column:
            input_column_index = self.record_info_in.get_field_num(
                input_column)
